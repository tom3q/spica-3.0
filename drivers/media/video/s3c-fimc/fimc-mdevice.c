/*
 * S3C/EXYNOS4 SoC series camera host interface media device driver
 *
 ...
 * @prep: true to acquire sensor subdevs
 *
...
{
	struct s3c_platform_fimc *pdata = fmd->pdev->dev.platform_data;
...
/*
 * FIMC platform devices registration.
 */
...
/**
 * fimc_md_register_platform_entities - register FIMC media entities
 */
 ...
 * entities. Enabled links are created by default between each subsequent
 * registered sensor and subsequent FIMC capture entity. The number of default
 * active links is determined by the number of available sensors or
 * FIMC entities whichever is less.
 */
static int fimc_md_create_links(struct fimc_md *fmd)
{
	struct v4l2_subdev *sensor;
	struct s3c_fimc_isp_info *pdata;
	struct fimc_sensor_info *s_info;
...
static int __fimc_md_set_camclk(struct fimc_md *fmd,
					 ...
					 bool on)
{
	struct s3c_fimc_isp_info *pdata = s_info->pdata;
 ...
 * devices to which sensors can be attached. The clock is allowed here 
 * to be used by multiple sensors concurrently if they use same frequency.
...
static int __devinit fimc_md_probe(struct platform_device *pdev)
{
	...
	strlcpy(fmd->media_dev.model, "SAMSUNG S3C FIMC",
...
MODULE_DESCRIPTION("S3C FIMC camera host interface/video postprocessor driver");
MODULE_LICENSE("GPL");
