 ...
 *  		  2011 Tomasz Figa <tomasz.figa at gmail.com> (Driver fixes)
 *
...
static void determine_jack_type(struct sec_jack_info *hi)
{
	...
	while (gpio_get_value(hi->pdata->det_gpio) ^ npolarity) {
		adc = s3c_adc_read(hi->adc, hi->pdata->adc_chan);
		pr_debug("%s: adc = %d\n", __func__, adc);
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	if (pdata->adc_chan < 0 || !pdata->zones ||
	...
	sec_jack_key_info.debounce_time = ktime_set(0,
					SEND_KEY_CHECK_TIME_MS * NSEC_PER_MSEC);
	if (pdata->send_end_active_high)
		sec_jack_key_info.flags = GPIOEDF_ACTIVE_HIGH;
	...
	hi->adc = s3c_adc_register(pdev, NULL, NULL, 0);
	if (IS_ERR(hi->adc)) {
